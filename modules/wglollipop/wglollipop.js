    ...
    function: function (div, row) {
      ...
      function drawMain() {
        ...
        var widgetContentDiv = document.getElementById(
          "widgetcontentdiv_" + widgetName + "_" + self.variables.tabName
        );
        var spinner =
        ...
        if (v.variantdatasource != null) {
          var select = document.getElementById(getVarDatasourceSelectorId());
        ...
        if (v.variantcategory != null) {
          var select = document.getElementById(getVarCategorySelectorId());
        ...
        if (v.sitedatasource != null) {
          var select = document.getElementById(getProtSiteSourceSelectorId());
      ...
      function getMyVariant() {
        ...
        var allMappings = JSON.parse(
          getWidgetData(tabName, "base", row, "all_mappings")
        );
      ...
      function onChangeOtherVariantDatasource(select, data) {
        ...
        var categories = null;
        var categoryPartDiv = document.getElementById(
          getVarCategorySelectorDivId()
        );
        if (datasource == "") {
          categories = [];
      ...
      function onChangeOtherVariantCategory(
        ...
        var y = 0;
        var varHeightInc =
          (v.varHeightMax - v.varHeightMin) / v.maxVarNumSample;
        ...
        for (var j = 0; j < variants.length; j++) {
          ...
          var rect = stage.rect(x - width / 2, y, width, height).fill(color);
          var circle = stage.circle(x, y + height, v.variantRadius).fill(color);
      ...
      function onChangeProtSiteDatasource(canvasId, data, datasource) {
        var v = widgetGenerators[widgetName][tabName]["variables"];
        ...
        dsDiv.style.height =
          maxStackLevel * (v.domainLineHeight + 2) + 5 + "px";
      }
      ...
      function drawMyVariant(variant, stage, y, varHeightInc) {
        ...
        var height = Math.min(
          v.varHeightMax,
          Math.max(v.varHeightMin, variant.count * varHeightInc)
        );
        ...
        var rect = stage
          .rect(x - width / 2, y + v.varHeightMax - height, width, height)
          .fill(color);
        var circle = stage
          .circle(x, y + v.varHeightMax - height, v.variantRadius)
          .fill(color);
      ...
      function drawBoxDomain(stage, domain, y) {
        ...
        var color = "brown";
        var box = stage
          .rect(x, y, width, v.boxDomainHeight)
      ...
      function drawLineDomain(stage, domain, y) {
        ...
        var line = stage
          .rect(x, y, width, v.domainLineHeight)
      ...
      function draw(data) {
        ...
        v.proteinWidth =
          parseInt(v.proteinWidth.substring(0, v.proteinWidth.length - 2)) -
          v.variantRadius -
          v.xEndPad;
        v.aalen = data["len"];
      ...
      function drawProtein(data) {
        ...
        // Protein
        stage
          .rect(v.xStart, y, v.proteinWidth, v.proteinHeight)
          .round(v.boxR, v.boxR, v.boxR, v.boxR)
          .stroke("#555555")
          .fill({
    ...
    function: function (div, row) {
      ...
      function drawMyVariantsTable(data) {
        ...
        var contentDiv = document.getElementById(
          "widgetcontentdiv_lollipop_gene"
        );
        ...
        for (var i = 0; i < myVariants.length; i++) {
          ...
          td.addEventListener("mouseover", function (evt) {
            ...
            var circles = $(
              'circle.lollipop_gene[aachange="' + aachange + '"]'
            );
      ...
      async function drawMyVariants(data) {
        var v = widgetGenerators[widgetName][tabName]["variables"];
        ...
        for (var i = 0; i < rows.length; i++) {
          var row = rows[i];
          var achange = row[variantdbcols["base__achange"]];
          if (achange == null) {
            continue;
          }
          var variant = getMyVariant(row, hugo);
      ...
      function getMyVariant(row, refHugo) {
        var v = widgetGenerators[widgetName][tabName]["variables"];
        var soPoints = v["soPoints"];
        var allMappings = JSON.parse(row[variantdbcols["base__all_mappings"]]);
        var hugos = Object.keys(allMappings);
        variant = {};
        for (var i = 0; i < hugos.length; i++) {
          var hugo = hugos[i];
          ...
          for (var j = 0; j < uniprot_ds.length; j++) {
            ...
            if (v.reftrNoVers.indexOf(transcript.split(".")[0]) >= 0) {
              ...
              variant["achange"] = protchange;
              variant["count"] = row[variantdbcols["tagsampler__numsample"]];
              break;
            }
          }
        }
