...
def convert_codon_to_codonnum(codon: str):
    codonnum = 0
...
class Mapper(BaseMapper):
    def extra_setup(self):
        self.add_additional_crx_def()

    def add_additional_crx_def(self):
        from json import dumps

        if not self.crx_writer or not self.conf:
            return
        refseq_col = {
            "name": "refseq",
            "title": "RefSeq",
        ...
        self.crx_writer.add_column(refseq_col)
        self.crx_writer.write_meta_line("column", dumps(refseq_col))
    ...
    def map(self, crv_data):
        ...
        for tr_map_start in tr_map_starts:
            ...
            if (
                ...
                if transcripttypeno == TRANSCRIPTTYPENO_NMD:
                    so += (SO_NMD,)
                mapping = (uniprot, achange, so, tr, cchange, alen, genename, coding, exonno + 1)
        ...
        crx_data = {x["name"]: "" for x in crx_def}
        ...
        crx_data["exonno"] = primary_mapping[MAPPING_EXONNO_I]
        amd = {}
            ...
            for mapping in all_mappings[genename]:
                amd[genename].append(
                    (
                        ...
                        mapping[MAPPING_EXONNO_I],
                    )
                )
        crx_data["all_mappings"] = json.dumps(amd)
        transcript_nover = crx_data["transcript"].split(".")[0]
        if transcript_nover in self.enst_nm:
            crx_data["refseq"] = self.enst_nm[transcript_nover]
        else:
            crx_data["refseq"] = ""
        return crx_data
    ...
    def _make_primary_transcripts(self):
        ...
        for primary_transcript_path in self.primary_transcript_paths:
            if primary_transcript_path in ["mane", "mane-select", "mane-clinical"]:
                self.enst_nm = {}
                ...
                nm_colno = toks.index("RefSeq_nuc")
                mane_status_colno = toks.index("MANE_status")
                for line in f:
                    toks = line[:-1].split("\t")
                    hugo = toks[hugo_colno]
                    enst = toks[enst_colno].split(".")[0]
                    nm = toks[nm_colno].split(".")[0]
                    self.enst_nm[enst] = nm
                    add = False
                    if hugo not in self.primary_transcript:
                        add = True
                    else:
                        status = toks[mane_status_colno]
                        if primary_transcript_path == "mane-clinical" and status == "MANE Plus Clinical":
                            add = True
                        elif primary_transcript_path in ["mane", "mane-select"] and status == "MANE Select":
                            add = True
                    if add:
    ...
    def setup(self):
        ...
        db_path = os.path.join(data_dir, "gene_41_10000.sqlite")
        ...
        mrnas_path = os.path.join(data_dir, "mrnas_prots_41.pickle")
        f = open(mrnas_path, "rb")
        self.mrnas = pickle.load(f)
        self.prots = pickle.load(f)
        f.close()
        self.hg38reader = get_wgs_reader(assembly="hg38")
        self._make_tr_info()
    ...
    def _get_tr_map_data(self, chrom, gpos):
        gposbin = int(gpos / self.binsize)
        q = f"""
            select * from "transcript_frags_{chrom}" where binno=? and start<=? and end>=? order by tid
            """
        try:
            self.c.execute(q, (gposbin, gpos, gpos))
            ret = self.c.fetchall()
    ...
    def _get_svn_cds_so(self, tid, _, cstart, tpos, tstart, alt_base, apos):
        [seq, ex] = self.mrnas[tid]
        # cpos_codonstart = int((cpos - 1) / 3) * 3 + 1
    ...
    def _get_primary_mapping(self, all_mappings):
        ...
        for hugo, mapping in primary_mappings.items():
            if primary_mapping[MAPPING_GENENAME_I] not in self.primary_transcript:
                if mapping[MAPPING_GENENAME_I] in self.primary_transcript:
    ...
    def summarize_by_gene(self, hugo, input_data):
        ...
        if "so" in input_data:
            so_key = "so"
            coding_key = "coding"
            all_mappings_key = "all_mappings"
            numsample_key = "numsample"
        else:
            so_key = "base__so"
            coding_key = "base__coding"
            all_mappings_key = "base__all_mappings"
            numsample_key = "tagsampler__numsample"
        sonums = [so_to_sonum[so] for so in set(input_data[so_key])]
        out["so"] = sonum_to_so[max(sonums)]
        out["num_noncoding_variants"] = len(
            [c for c in input_data[coding_key] if c != "Y"]
        )
        out["num_coding_variants"] = len([c for c in input_data[coding_key] if c == "Y"])
        so_counts = {}
        all_mappings_lines = input_data[all_mappings_key]
        for lineno in range(len(all_mappings_lines)):
            line = all_mappings_lines[lineno]
            all_mappings = json.loads(line)
            numsample = input_data[numsample_key][lineno]
